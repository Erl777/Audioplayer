...
class Audioplayer {
    constructor(params) {
        ...
        this.input = document.getElementById('seek');
        this.song = null;
        this.player = document.getElementById('audio');
        this.audioElem = null; // сам трек
        this.id = 0;
        ...
        this.volumeInput = document.getElementById('volume-mobile');
        this.albumNameElem = document.querySelector('.album-name');
        this.numberOfSongsElem = document.querySelector('.number-of-songs');
        setTimeout(() => {
    ...
    async initialization (){
        ...
        /*  Добавление трека */
        this.addAudioElem();
        this.audioElem = document.getElementById('track');
        ...
        this.changeNumberOfSongs();

        this.volumeInput.addEventListener('input', this.changeVolume);
        // устанавливаю значение звука по умолчанию
        this.audioElem.volume = 0.5;
        console.log(this.audioElem.volume);
        // устанавливаю ползунок звука в установленное значение звука по умолчанию
        this.volumeInput.value = this.audioElem.volume * 100;

        // подставляю название первой песни из первого плейлиста 
        document.getElementById('trackName').textContent = this.settings.playlist[this.getShownPlaylistName()][0].name;
        // выставление нужной svg в зависимости от состояния loop переменной
        this.loopToggle(true);
        this.randomSongToggle(true);
    }

    changeNumberOfSongs(){
        // let shownPlaylist = this.getShownPlaylistName();
        // console.log(this.settings.playlist[shownPlaylist].length);
        let numberOfSongs = this.settings.playlist[this.getShownPlaylistName()].length
        this.numberOfSongsElem.textContent = numberOfSongs + ' песен';
    }

    changePlaylistName(){
        this.albumNameElem.textContent = this.getShownPlaylistName();
    }

    changeVolume = () => {
        let volumeValue = this.volumeInput.value / 100;
        volumeValue = volumeValue.toPrecision(1);
        volumeValue = Number(volumeValue);
        // console.log(volumeValue);
        // console.log(this.player);
        try{
            if(this.audioElem.value !== volumeValue) {
                // console.log(this.player.volume);
                this.audioElem.volume = volumeValue;
                // console.log(volumeValue, typeof volumeValue);
            }
        }
        catch (e) {
            console.log(e);
    ...
    loopToggle = (checkWithoutToggle) => {
        if(checkWithoutToggle === true){
            console.log('loop ignored');
        }
        else {
            // переключение параметра loop у аудио элемента
            this.settings.loop = this.settings.loop !== true;
        }
        let imgArr = this.loop.querySelectorAll('.img-relative');
        if( this.settings.loop === true && !imgArr[0].classList.contains('active')){
            imgArr[0].classList.add('active');
            imgArr[1].classList.remove('active');
        }
        if (this.settings.loop === false && !imgArr[1].classList.contains('active')){
    ...
    randomSongToggle = (checkWithoutToggle) => {
        if(checkWithoutToggle === true){
            console.log('random ignored');
        }
        else {
            // переключение параметра randomSong
            this.settings.randomSong = this.settings.randomSong !== true;
        }
        let imgArr = this.randomSongElem.querySelectorAll('.img-relative');
        if( this.settings.randomSong === true && !imgArr[0].classList.contains('active')){
            imgArr[0].classList.add('active');
            imgArr[1].classList.remove('active');
        }
        if (this.settings.randomSong === false && !imgArr[1].classList.contains('active')){
            imgArr[1].classList.add('active');
            imgArr[0].classList.remove('active');
        }
    };
    ...
    startRandomSong ()  {
        ...
        if(this.settings.randomSong === true){
            ...
            let shownSongsArr = document.querySelectorAll('li[data-song-id]');
    ...
    generatePlaylists () {
        ...
        let playlistsContainer = document.querySelector('.albums-list');
